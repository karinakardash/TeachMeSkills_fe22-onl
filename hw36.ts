...
этой функции должен быть отфильтрованный массив, где параметры 2 и 3 равны в объекте фильма. Например: передаем
*/

//Task 1

function getGenres(arr: typeof films) {
    const genres = arr.flatMap(item => item.genre);
...
function getActors(arr: typeof films) {
    const actors = arr.flatMap(item => item.actors);
...
function sortByRating (arr: typeof films) {
    return arr.sort((a, b) => {
...
function returnNewArr (arr: typeof films) {
    return arr.map (item => {
        return {
            id: item.id,
            title: item.title,
...
function filterByYear (arr: typeof films, year: number) {
    return arr.filter((item) => year === item.year);
}

filterByYear(films, 2011);

//Task 6

function filterByTitle (arr: typeof films, str: string) {
    return arr.filter((item) => item.title.toUpperCase().includes(str.toUpperCase()));
}

filterByTitle(films, "Sorcerer's Stone");

//Task 7

function filterByTitleOrPlot (arr: typeof films, str: string) {
    return arr.filter((item) => (item.title.toUpperCase().includes(str.toUpperCase())) || (item.plot.toUpperCase().includes(str.toUpperCase())));
}

filterByTitleOrPlot(films, "Natasha Romanoff");

//Task 8

function searchByProperties(arr: typeof films, str: string, value: string | number) {

    if(typeof value === "string") {
        return arr.filter(item => {
            if (item.hasOwnProperty(str) && (item[str] === value || item[str].includes(value))) {
                return item
            }
    })
    } else if (typeof value === "number") {
        return arr.filter(item => {
            if (item.hasOwnProperty(str) && item[str] === value) {
                return item
            }
    })
    }
...
searchByProperties(films, "year", 2021)